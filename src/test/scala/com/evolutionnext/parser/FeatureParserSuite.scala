...
import com.evolutionnext.gherkin.{Feature, Step, Tag}
...
class FeatureParserSuite extends munit.FunSuite {
  ...
  test("parse 002-data-tables.feature") {
    ...
    val givenStep: Step = feature.scenarios.head.steps.filter(s => s.keyword == Given).head
    val table = givenStep.table match {
      case Some(table) => table
      case None => fail("Did not contain a table")
    }
